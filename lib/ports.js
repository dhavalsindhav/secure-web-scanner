...
const net = safeRequire('net', createUnavailableFeatureProxy('net'));
const portScanner = safeRequire('node-port-scanner', createUnavailableFeatureProxy('node-port-scanner'));
  ...
  if ([80, 443, 8000, 8080, 8443, 3000, 8888, 9000, 9090].includes(port)) {
    try {
      ...
      const response = await axios.get(`${protocol}://${host}:${port}`, {
        ...
        maxRedirects: 0,
        headers: {
          'User-Agent': 'Mozilla/5.0 secure-web-scanner/1.0.1'
        }
      });
